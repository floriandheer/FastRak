"""Outgoing invoices tab — creation, numbering, and registry management.

Adapted from PipelineScript_Business_GlobalInvoice.py.
...
from global_invoice.legacy_scanner import (
    ScannedInvoice,
    PATTERN_UNRECOGNISED,
    scan_boekhouding,
)
...
class OutgoingInvoicesTab:
    """Outgoing invoices management panel — embeds inside a tab frame."""
    ...
    def __init__(self, parent: tk.Frame):
        ...
        try:
            ...
            C = FORM_COLORS
            err_frame = tk.Frame(parent, bg=C["bg"])
            err_frame.pack(fill="both", expand=True, padx=20, pady=20)
            tk.Label(
                err_frame,
                text=f"Cannot load outgoing invoice config:\n\n{e}",
                fg=C["warning"], bg=C["bg"], font=("Arial", 11),
                justify="left",
            ).pack(anchor="w")
            return
    ...
    def _build_styles(self):
        ...
        style.configure("OutgoingInvoice.TNotebook", background=C["bg"])
        style.configure(
            "OutgoingInvoice.TNotebook.Tab",
            background=C["bg_input"], foreground=C["text"],
            padding=[14, 6],
        )
        style.map(
    ...
    def _build_ui(self):
        ...
        # DB info bar
        info_bar = tk.Frame(self.root, bg=C["bg"], pady=4)
        info_bar.pack(fill="x", padx=12)
        tk.Label(
            info_bar,
            text=f"DB: {self.config.resolve_db_path()}",
            font=("Arial", 9), fg=C["text_dim"], bg=C["bg"],
        ).pack(side="left")

        # Debug-mode banner (hidden unless a debug session is active)
        self._debug_banner = tk.Frame(self.root, bg="#b91c1c", height=32)
        self._debug_banner_label = tk.Label(
            self._debug_banner, text="",
            font=("Arial", 11, "bold"), fg="white", bg="#b91c1c",
        )
        ...
        # Sub-notebook
        self.notebook = ttk.Notebook(self.root, style="OutgoingInvoice.TNotebook")
    ...
    def _build_dashboard_tab(self):
        ...
        years = [str(y) for y in (self.registry.list_years() or [datetime.now().year])]
        ...
        self._mk_button(ctrl, "🔄  Sync from WooCommerce",
                        self._sync_from_woocommerce).pack(side="left", padx=(8, 0))

        tree_frame = tk.Frame(tab, bg=C["bg"])
        tree_frame.pack(fill="both", expand=True, padx=8, pady=4)

        cols = ("number", "year", "company", "date", "customer", "total", "status", "source")
        self.dash_tree = ttk.Treeview(
            tree_frame, columns=cols, show="headings",
            style="OutgoingInvoice.Treeview",
        )
        for c, w, anchor in [
            ("number", 80, "center"),
            ("year", 60, "center"),
    ...
    def _refresh_dashboard(self):
        ...
        company = self.dash_company.get() if self.dash_company.get() != "(all)" else None
        status = self.dash_status.get() if self.dash_status.get() != "(all)" else None
        search = self.dash_search.get().strip() or None

        for iid in self.dash_tree.get_children():
            self.dash_tree.delete(iid)

        rows = self.registry.list_invoices(year=year, company_key=company,
    ...
    def _sync_from_woocommerce(self):
        ...
        ok = messagebox.askyesno(
            ...
            "already have a WCPDF invoice number into the registry.\n\n"
        ...
        self._set_busy(True)
        threading.Thread(
            target=self._sync_worker, args=(bridge, default_vat_rate), daemon=True,
        ).start()
    ...
    def _on_sync_done(self, report, error: Optional[str]):
        if error:
            messagebox.showerror("Sync from WooCommerce", f"Sync failed:\n\n{error}")
            return
        self._refresh_dashboard()
    ...
    def _build_new_invoice_tab(self):
        ...
        tk.Spinbox(
            ...
            bg=C["bg_input"], fg=C["text"], insertbackground=C["text"], relief=tk.FLAT,
        ...
        self.li_tree = ttk.Treeview(
            ...
            style="OutgoingInvoice.Treeview", height=7,
        )
        for c, w, anchor in [
            ("description", 280, "w"),
            ("qty", 60, "e"),
            ...
            self.li_tree.column(c, width=w, anchor=anchor, stretch=(c == "description"))
        li_sb = ttk.Scrollbar(li_frame, orient="vertical", command=self.li_tree.yview)
        ...
        self._mk_button(li_btns, "Edit", self._edit_line_item).pack(side="left", padx=(6, 0))
        self._mk_button(li_btns, "Remove", self._remove_line_item).pack(side="left", padx=(6, 0))
        ...
        self.exp_tree = ttk.Treeview(
            ...
            style="OutgoingInvoice.Treeview", height=5,
        )
        for c, w, anchor in [
            ("description", 280, "w"),
            ("qty", 60, "e"),
            ...
            self.exp_tree.column(c, width=w, anchor=anchor, stretch=(c == "description"))
        ...
        self._mk_button(exp_btns, "Edit", self._edit_expense_item).pack(side="left", padx=(6, 0))
        self._mk_button(exp_btns, "Remove",
                        self._remove_expense_item).pack(side="left", padx=(6, 0))
    ...
    def _edit_line_item(self):
        ...
        if not sel:
            return
        idx = int(sel[0])
        item = LineItemDialog(self.root, existing=self._line_items[idx]).result
        if item is None:
            return
    ...
    def _generate_invoice_worker(
        ...
        try:
            ...
            logger.info(
                f"Reserved invoice #{sequence:03d} ({year}) for {company.key} / {customer}"
            )
    ...
    def _build_template_context(self, company: Company, row: Dict[str, Any]) -> Dict[str, str]:
    ...
    def _build_legacy_tab(self):
        C = FORM_COLORS
        tab = tk.Frame(self.notebook, bg=C["bg"])
        self.notebook.add(tab, text="📂  Legacy")

        self._legacy_results: List[ScannedInvoice] = []
        self._legacy_registry_status: Dict[str, str] = {}

        ctrl = tk.Frame(tab, bg=C["bg"])
        ctrl.pack(fill="x", padx=8, pady=(8, 4))

        self._mk_button(ctrl, "🔍 Scan now", self._scan_legacy_start).pack(side="left")

        tk.Label(ctrl, text="Year:", fg=C["text"], bg=C["bg"]).pack(side="left", padx=(16, 0))
        self._legacy_year = tk.StringVar(value="(all)")
        self._legacy_year_combo = ttk.Combobox(
            ctrl, textvariable=self._legacy_year,
            values=["(all)"], width=8, state="readonly",
        )
        self._legacy_year_combo.pack(side="left", padx=(4, 12))

        tk.Label(ctrl, text="Pattern:", fg=C["text"], bg=C["bg"]).pack(side="left")
        self._legacy_pattern = tk.StringVar(value="(all)")
        ttk.Combobox(
            ctrl, textvariable=self._legacy_pattern,
            values=["(all)", "canonical", "prefixed_legacy",
                    "long_date_legacy", "unrecognised"],
            width=16, state="readonly",
        ).pack(side="left", padx=(4, 12))

        tk.Label(ctrl, text="Status:", fg=C["text"], bg=C["bg"]).pack(side="left")
        self._legacy_status_filter = tk.StringVar(value="(all)")
        ttk.Combobox(
            ctrl, textvariable=self._legacy_status_filter,
            values=["(all)", "new", "in registry"],
            width=12, state="readonly",
        ).pack(side="left", padx=(4, 12))

        self._mk_button(ctrl, "Apply", self._apply_legacy_filter).pack(side="left")

        self._legacy_status_label = tk.Label(
            ctrl, text="", fg=C["text_dim"], bg=C["bg"], font=("Arial", 9),
        )
        self._legacy_status_label.pack(side="right", padx=8)

        tree_frame = tk.Frame(tab, bg=C["bg"])
        tree_frame.pack(fill="both", expand=True, padx=8, pady=4)

        cols = ("year", "q", "seq", "date", "company", "description", "pattern", "status")
        self.legacy_tree = ttk.Treeview(
            tree_frame, columns=cols, show="headings",
            style="OutgoingInvoice.Treeview", selectmode="extended",
        )
        for col, width, anchor in [
            ...
            self.legacy_tree.heading(col, text=col.capitalize())
            self.legacy_tree.column(col, width=width, anchor=anchor,
                                    stretch=(col == "description"))

        self.legacy_tree.tag_configure("in_registry", foreground="#3fb950")
        self.legacy_tree.tag_configure("new", foreground=C["text"])
        self.legacy_tree.tag_configure("duplicate", foreground="#d29922")
        self.legacy_tree.tag_configure("no_import", foreground="#8b949e")

        sb = ttk.Scrollbar(tree_frame, orient="vertical", command=self.legacy_tree.yview)
        self.legacy_tree.configure(yscrollcommand=sb.set)
        self.legacy_tree.pack(side="left", fill="both", expand=True)
        sb.pack(side="right", fill="y")
        self.legacy_tree.bind("<Double-1>", lambda e: self._open_legacy_pdf())

        actions = tk.Frame(tab, bg=C["bg"])
        actions.pack(fill="x", padx=8, pady=(4, 8))
        self._mk_button(actions, "Open PDF", self._open_legacy_pdf).pack(side="left")
        self._mk_button(actions, "Open folder",
                        self._open_legacy_folder).pack(side="left", padx=(8, 0))
        self._mk_button(actions, "Import selected → registry",
                        self._import_selected_legacy).pack(side="left", padx=(8, 0))
        tk.Label(
            actions,
            text="Import creates stub registry entries (no amounts). "
                 "Unrecognised files must be renamed first.",
            fg=C["text_dim"], bg=C["bg"], font=("Arial", 9),
        ).pack(side="left", padx=(16, 0))

    def _scan_legacy_start(self):
        self._legacy_status_label.configure(text="Scanning…")
        self._set_busy(True)
        threading.Thread(target=self._scan_legacy_worker, daemon=True).start()

    def _scan_legacy_worker(self):
        try:
            ...
            known_slots = {
                (inv["year"], inv["company_key"], inv["sequence"])
                for inv in all_inv
            }
            reg_status: Dict[str, str] = {}
            for r in results:
                pk = str(r.path)
                if pk in known_paths:
                    reg_status[pk] = "in registry"
                elif (r.year, r.company_key, r.sequence) in known_slots:
                    reg_status[pk] = "in registry"
                else:
                    reg_status[pk] = "new"

            self.root.after(0, self._on_legacy_scan_done, results, reg_status, None)
        except Exception as e:
            logger.exception("Legacy scan failed")
            self.root.after(0, self._on_legacy_scan_done, [], {}, str(e))
        finally:
            self.root.after(0, self._set_busy, False)

    def _on_legacy_scan_done(
        self,
        ...
        if error:
            messagebox.showerror("Legacy scan", f"Scan failed:\n\n{error}")
            self._legacy_status_label.configure(text="Scan failed.")
            return

        self._legacy_results = results
        self._legacy_registry_status = reg_status

        years = sorted({r.year for r in results}, reverse=True)
        self._legacy_year_combo.configure(values=["(all)"] + [str(y) for y in years])
        self._apply_legacy_filter()

    def _apply_legacy_filter(self):
        year_s = self._legacy_year.get()
        year_f = int(year_s) if year_s.isdigit() else None
        pat_f = self._legacy_pattern.get()
        if pat_f == "(all)":
            pat_f = None
        stat_f = self._legacy_status_filter.get()
        if stat_f == "(all)":
            stat_f = None

        filtered = [
            r for r in self._legacy_results
            if (year_f is None or r.year == year_f)
            and (pat_f is None or r.pattern == pat_f)
            and (stat_f is None
                 or self._legacy_registry_status.get(str(r.path)) == stat_f)
        ]

        for iid in self.legacy_tree.get_children():
            self.legacy_tree.delete(iid)

        for r in filtered:
            rs = self._legacy_registry_status.get(str(r.path), "new")
            status_label = "✅ imported" if rs == "in registry" else "new"
            desc = r.description
            if r.notes:
                desc = f"⚠  {desc}"
            ...
            if r.notes:
                ...
                tag = "in_registry"
            elif r.pattern == PATTERN_UNRECOGNISED:
                tag = "no_import"
            else:
                tag = "new"
            ...
            self.legacy_tree.insert(
                "", "end", iid=str(r.path),
                values=(
                    r.year, f"Q{r.quarter}",
                    f"{r.sequence:03d}" if r.sequence is not None else "—",
                    r.invoice_date or "—",
                    r.company_key or "—",
                    desc, r.pattern, status_label,
                ),
                tags=(tag,),
            )

        total = len(self._legacy_results)
        shown = len(filtered)
        new_count = sum(
            1 for r in self._legacy_results
            if self._legacy_registry_status.get(str(r.path)) == "new"
        )
        self._legacy_status_label.configure(
            text=f"{shown}/{total} shown · {new_count} not yet imported"
        )

    def _selected_legacy_invoices(self) -> List[ScannedInvoice]:
        sel = self.legacy_tree.selection()
        path_set = set(sel)
        return [r for r in self._legacy_results if str(r.path) in path_set]

    def _open_legacy_pdf(self):
        items = self._selected_legacy_invoices()
        if items:
            _open_path(items[0].path)

    def _open_legacy_folder(self):
        items = self._selected_legacy_invoices()
        if items:
            _open_path(items[0].path.parent)

    def _import_selected_legacy(self):
        items = self._selected_legacy_invoices()
        if not items:
            messagebox.showinfo("Import", "Select one or more invoices first.")
            return
        ...
        if not importable:
            messagebox.showinfo(
                "Import",
                "None of the selected invoices can be auto-imported.\n\n"
                "Unrecognised files need a proper filename first.",
            )
            return

        already = [
            r for r in importable
            if self._legacy_registry_status.get(str(r.path)) == "in registry"
        ]
        to_import = [
            r for r in importable
            if self._legacy_registry_status.get(str(r.path)) != "in registry"
        ...
        if not to_import:
            messagebox.showinfo("Import", "All selected are already in the registry.")
            return
        ...
        for r in to_import:
            draft = {
                ...
                "customer_name": r.import_customer_name,
                "line_items": [],
                "source": "legacy_scan",
                "source_ref": str(r.path),
                "notes": r.notes or "",
            }
            try:
                self.registry.import_existing_invoice(
                    r.year, r.sequence, draft, pdf_path=str(r.path)
                )
                successes.append(r)
                self._legacy_registry_status[str(r.path)] = "in registry"
    ...
    def _refresh_debug_ui(self):
        ...
        if hasattr(self, "_enter_debug_btn"):
            self._enter_debug_btn.configure(state=("disabled" if active else "normal"))
            self._exit_debug_btn.configure(state=("normal" if active else "disabled"))
    ...
    def _enter_debug_mode(self):
        ...
        try:
            ...
            messagebox.showerror("Enter debug mode",
                                 f"Could not start debug session:\n\n{e}")
            return
        self._refresh_debug_ui()
        messagebox.showinfo(
            "Debug mode",
