...
from global_invoice.debug_mode import debug_boekhouding_base
...
from invoices_app.widgets.inputs import make_entry, make_spinbox, make_text, form_label
from invoices_app.widgets.line_item_dialog import LineItemDialog
from invoices_app.widgets.tree import make_treeview

logger = get_logger("invoices_app.compose")
...
class ComposeSection(Section):
    ...
    def _wc_worker(self, bridge, customer, desc, amount_str, project_folder):
        try:
            ...
            if project_folder:
                from global_invoice.wc_bridge import extract_wc_invoice_info
                from global_invoice.filer import file_pdf as _file_pdf, create_shortcut
                boek = self.state.config.resolve_boekhouding_base()
