"""
Audio Project Folder Structure Creator

Creates standardized folder structure for audio/music projects.
...
class FolderStructureCreator(FormKeyboardMixin):
    """Creates folder structure for audio projects with keyboard-first navigation."""
    ...
    def __init__(self, root_or_frame, embedded=False, on_project_created=None, on_cancel=None, project_db=None):
        ...
        if embedded:
            ...
            self.parent = root_or_frame
            self.root.title("Audio Folder Structure")
    ...
    def _build_form(self):
        ...
        if self.project_db:
            self.client_entry = AutocompleteEntry(
                row1, db=self.project_db, category="Audio",
        ...
        sw_defaults = self.settings.get_software_defaults("Audio")

        # Software chips for DAWs
        self.software_chip_frame, self.software_chips = create_software_chip_row(
            row2,
            ["Ableton", "Reaper"],
            defaults={
                "Ableton": sw_defaults.get("ableton", "12"),
                "Reaper": sw_defaults.get("reaper", "7"),
            },
        ...
        default_base = self.settings.get_work_path("Audio").replace('\\', '/')
        self.base_dir_var = tk.StringVar(value=default_base)
        self.template_dir_var = tk.StringVar(value=os.path.join(TEMPLATES_DIR, 'YYYY-MM-DD_AudioProd_NameClient_NameProject'))
    ...
    def update_preview(self):
        ...
        if active_software:
            ...
            self.preview_text.insert(tk.END, "No DAW selected\n\n")

        template_structure = self.get_template_structure(self.template_dir_var.get())
